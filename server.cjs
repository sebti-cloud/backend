...
const pool = new Pool({
  ...
  ssl: { rejectUnauthorized: false }  // Ajout de la connexion SSL
});
...
app.put('/api/update-tracking/:orderId', async (req, res) => {
  ...
  console.log('Received data:', { orderId, status, paymentMethod });
...
app.post('/api/purchases', (req, res) => {
  ...
  pool.query(query, [userId, product])
    .then(result => {
      console.log('Achat enregistré:', { userId, product });
